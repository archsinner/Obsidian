...
Rust doesn’t let us call `drop` explicitly because Rust would still automatically call `drop` on the value at the end of `main`. This would be a _double free_ error because Rust would be trying to clean up the same value twice.
